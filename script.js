...
function saveGame() {
    ...
    const saveData = { pet, backpack, pot };
    localStorage.setItem('my_pet_game_save', JSON.stringify(saveData));
    alert("💾 游戏进度已成功保存在当前浏览器！");
}

// 📂 读取本地存档
function loadGame() {
    const saved = localStorage.getItem('my_pet_game_save');
    if (saved) {
        try {
...
window.addEventListener('load', () => {
    const saved = localStorage.getItem('my_pet_game_save');
    if (saved) {
        if (confirm("检测到你有上次保存的游戏进度，是否读取存档？")) {
            loadGame();
        }
    }
});